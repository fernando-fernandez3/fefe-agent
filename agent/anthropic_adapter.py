...
def build_anthropic_client(api_key: str, base_url: str = None):
    ...
    # Granular timeouts: read=300s matches the watchdog threshold so the SDK
    # surfaces a timeout error before the watchdog has to forcibly close the
    # connection.  max_retries=0 hands retry policy to Hermes's outer loop
    # (which has credential rotation and provider fallback the SDK lacks).
    kwargs = {
        "timeout": Timeout(connect=10.0, read=300.0, write=30.0, pool=30.0),
        "max_retries": 0,
    }
