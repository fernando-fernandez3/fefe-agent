...
For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Hermes ships with memory provider plugins — including native Long Memory, Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

Memory providers run **alongside** built-in memory (never replacing it) and add capabilities like local long-form notes, knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.
