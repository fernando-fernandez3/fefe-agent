...
class AIAgent:
    ...
    def _create_openai_client(
        ...
        # Inject timeout + retry defaults so codex/openrouter/custom providers
        # inherit the same hang protection as Anthropic.  Caller-provided values
        # win.  read=300s matches the non-streaming watchdog; max_retries=0 hands
        # retry policy to Hermes's outer loop (which has credential rotation +
        # provider fallback the SDK lacks).  Without this, a hung provider can
        # block for SDK-default 600s × 2 retries = ~20 min per "single" call.
        if "timeout" not in client_kwargs:
            try:
                import httpx as _httpx_to

                client_kwargs["timeout"] = _httpx_to.Timeout(
                    connect=10.0, read=300.0, write=30.0, pool=30.0
                )
            except Exception:
                pass
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        # Structured trace so post-hoc debugging can reconstruct hung calls
        # without depending on the stale-warning log line firing.
        logger.info(
            "non_streaming_api_call_start mode=%s model=%s est_tokens=%d stale_timeout=%.0fs",
            self.api_mode,
            api_kwargs.get("model", "unknown"),
            sum(len(str(v)) for v in api_kwargs.get("messages", [])) // 4,
            _stale_timeout,
        )
